...
class URLGenerator:
    ...
    @staticmethod
    def generate_canonical(calculator_type: str, country: str, state: str | None = None, city: str | None = None) -> str:
        """
...
class ContentGenerator:
    ...
    def _format_location(country: str, state: str | None, city: str | None) -> str:
        ...
        if state:
            parts.append(state.upper() if len(state) == 2 else state.title())
        if country:
            parts.append(country.upper() if len(country) == 2 else country.title())
        return ", ".join(parts)

    @staticmethod
    def generate_page_content(calculator_type: str, country: str, state: str | None, city: str | None, tax_year: int) -> dict[str, Any]:
