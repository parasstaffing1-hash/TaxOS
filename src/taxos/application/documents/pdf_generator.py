...
class PDFDocumentGenerator:
    ...
    def generate(
        ...
        primary_color_hex = getattr(
            getattr(template, "branding", None), "primary_color", "#000000"
        )
        primary_color = _hex_to_rgb(primary_color_hex)
        company_name = getattr(getattr(template, "branding", None), "company_name", "Company Name")
        footer_text = getattr(
            getattr(template, "branding", None), "footer_text", "Confidential - Do not distribute"
        )
        ...
        if qr_code_url_pattern:
            try:
                ...
                logger.warning("qr_code_rendering_failed", exc_info=True)
        ...
        if watermark_text:
            try:
                ...
                logger.warning("watermark_rendering_failed", exc_info=True)
            ...
            if getattr(section, "type", "") == "chart":
                ...
                try:
                    ...
                    pdf.cell(
                        0,
                        10,
                        "Chart rendering failed or not available.",
                        new_x="LMARGIN",
                        new_y="NEXT",
                    )
