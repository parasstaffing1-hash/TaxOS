...
from sqlalchemy import String, Boolean, JSON, ForeignKey, Integer, Text, Index
...
class SEORoute(Base):
    ...
    internal_links: Mapped[list["SEOInternalLink"]] = relationship(
        "SEOInternalLink", foreign_keys="SEOInternalLink.source_route_id", back_populates="source_route"
    )

    __table_args__ = (
        Index("idx_seo_location", "country", "state", "city"),
    )
...
class SEOInternalLink(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    source_route_id: Mapped[int] = mapped_column(ForeignKey("seo_routes.id", ondelete="CASCADE"), index=True)
    destination_slug: Mapped[str] = mapped_column(String(255))
    link_text: Mapped[str] = mapped_column(String(255))
    relationship_type: Mapped[str] = mapped_column(String(50)) # e.g. "sibling_city", "parent_state", "related_calculator"

    source_route: Mapped["SEORoute"] = relationship("SEORoute", back_populates="internal_links", foreign_keys=[source_route_id])
