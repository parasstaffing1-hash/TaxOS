...
from taxos.domain.rules import TaxRuleSet, ProgressiveTaxRule, TaxBracket, JurisdictionLevel
        ...
        rules={
            "single": [
                ProgressiveTaxRule(
                    ...
                    brackets=[
                        TaxBracket(min_amount=Decimal("0"), max_amount=Decimal("11600"), rate=Decimal("0.10")),
                        TaxBracket(min_amount=Decimal("11600"), max_amount=Decimal("47150"), rate=Decimal("0.12")),
                        TaxBracket(min_amount=Decimal("47150"), max_amount=None, rate=Decimal("0.22")),
                    ]
                )
            ]
        }
...
def test_validator_overlapping_brackets(base_ruleset):
    ...
    bad_bracket = TaxBracket(min_amount=Decimal("10000"), max_amount=Decimal("47150"), rate=Decimal("0.12"))
    new_rule = rule.model_copy(update={"brackets": [rule.brackets[0], bad_bracket, rule.brackets[2]]})
