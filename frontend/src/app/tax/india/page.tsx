      ...
      <div className="mx-auto max-w-[1180px]">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.slice(0, 60).map((tool) => {
            const isReady = tool.status === "complete" || tool.status === "partial";
            const statusLabel = tool.status ? tool.status.replace("_", " ").toUpperCase() : "PLANNED";
            return isReady ? (
              <Link key={tool.id} href={tool.route} className="group rounded-xl border border-[#e8e6e1] bg-white p-5 hover:border-[#cbc7be] hover:shadow-sm">
                <div className="flex items-center justify-between"><span className="font-mono text-[11px] text-[#9c978f]">#{String(tool.number).padStart(3, "0")}</span><span className="rounded bg-emerald-50 px-1.5 py-0.5 text-[10px] font-bold uppercase text-emerald-700">{statusLabel}</span></div>
              ...
              <div key={tool.id} className="rounded-xl border border-[#e8e6e1] bg-[#faf9f7] p-5 opacity-75">
                <div className="flex items-center justify-between"><span className="font-mono text-[11px] text-[#9c978f]">#{String(tool.number).padStart(3, "0")}</span><span className="rounded bg-stone-100 px-1.5 py-0.5 text-[10px] font-bold uppercase text-stone-500">{statusLabel}</span></div>
                <h2 className="mt-3 text-base font-semibold text-stone-600">{tool.title}</h2><p className="mt-1.5 line-clamp-3 text-xs leading-5 text-[#9c978f]">This workflow is not released yet. It will appear here when its tax rules, API, and tests are verified.</p>
              </div>
            );
          })}
        </div>
