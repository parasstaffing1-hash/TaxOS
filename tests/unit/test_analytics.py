...
from taxos.api.schemas.analytics import LocationComparisonRequest, IncomeDistributionRequest
from taxos.api.schemas.calculator import CalculatorRequest
from taxos.domain.financial.validation import LocationProfile, DemographicProfile, IncomeProfile
...
class MockSalaryService:
    async def calculate(self, request: CalculatorRequest):
        ...
        return CalculationResponse(
            gross_income=PeriodAmounts(annual=val, monthly=val/12, biweekly=val/26, weekly=val/52, daily=val/365, hourly=val/2080),
            net_income=PeriodAmounts(annual=net, monthly=net/12, biweekly=net/26, weekly=net/52, daily=net/365, hourly=net/2080),
            total_tax=tax,
