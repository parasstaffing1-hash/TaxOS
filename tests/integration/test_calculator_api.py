...
class TestCalculatorAPI:
    ...
    async def test_calculate_salary_json(self) -> None:
        ...
        payload = {
            "income": {"gross_income": "100000", "currency": "USD"},
            "location": {"country": "US"},
            "demographics": {"filing_status": FilingStatus.SINGLE.value, "tax_year": 2024},
        }
    ...
    async def test_calculate_salary_pdf(self) -> None:
        ...
        payload = {
            "income": {"gross_income": "50000", "currency": "USD"},
            "location": {"country": "US"},
            "demographics": {"filing_status": FilingStatus.SINGLE.value, "tax_year": 2024},
        }
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post(
                "/api/v1/after-tax-salary-calculator/US/pdf", json=payload
            )
    ...
    async def test_calculate_salary_excel(self) -> None:
        ...
        payload = {
            "income": {"gross_income": "75000", "currency": "USD"},
            "location": {"country": "US"},
            "demographics": {"filing_status": FilingStatus.SINGLE.value, "tax_year": 2024},
        }
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post(
                "/api/v1/after-tax-salary-calculator/US/excel", json=payload
            )
